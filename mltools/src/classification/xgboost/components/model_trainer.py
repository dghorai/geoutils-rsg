...
class ModelTrainerConfig:
    trained_model_filepath = os.path.join(PRJ_DIR, "artifacts", "classification", "xgboost", "model.pkl")
...
class ModelTrainer:
    ...
    def initiate_model_training(self, train_arrc, test_arrc):
        try:
            ...
            X_train, y_train, X_test, y_test = (train_arrc[:, :-1], train_arrc[:, -1], test_arrc[:, :-1], test_arrc[:, -1])
