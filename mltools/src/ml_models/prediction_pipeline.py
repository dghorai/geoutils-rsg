...
class PredictPipeline:
    ...
    def predict(self, features):
        try:
            # get file path
            preprocessor_file_path = os.path.join(PRJ_DIR, "artifacts", "regression", "random_forest", "preprocessor.pkl")
            model_file_path = os.path.join(PRJ_DIR, "artifacts", "regression", "random_forest", "model.pkl")
...
class CustomData:
    ...
    def format_user_data(self, user_data_dict):
        ...
        df = df[['CRIM', 'ZN', 'INDUS', 'CHAS', 'AGE', 'DIS', 'RAD', 'B', 'LSTAT']]
        # feature clipping
