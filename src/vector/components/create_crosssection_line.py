...
class XSCL:
    def __init__(self, plconfig: PolylineConfig):
        ...
        self.coordsys = self.plconfig.coordsys

    def construct_perpendicular_line(self, infc, outfc=None, interval=500, offset=500, save=True, coordsys='GCS'):
    ...
    def create_xscl_uniqueid(self):
        ...
        if self.xscl_available == True:
            ...
            self.construct_perpendicular_line(
                infc=self.infc, outfc=self.outfc, interval=self.interval, offset=self.offset, coordsys=self.coordsys)
            objids, lineids = self.construct_perpendicular_line(
                infc=self.infc, interval=self.interval, offset=self.offset, save=False, coordsys=self.coordsys)
