...
def segment_link(toconnect, fntnodeids):
...
def group_segments(ftnodes, segment_counts):
    ...
    for x in range(len(segment_counts)):
        xt = segment_link(toconnect, ftnodes)
        toconnect = xt
        if xt == []:
            break
        else:
    ...
    group_2 = [(k, [v[1] for v in g]) for k, g in groupby(sorted(toconnect5), key=itemgetter(0))]
    # return segment ids
    return group_1, group_2

def update_field(group_1, group_2, layer, feat, comid, fieldname):
    """Update field"""
    for s in group_1:
        if comid == s:
...
def find_line_terminals(tmnl2, mfl2):
...
def generate_hydroid(gdisc):
    ...
    for k in gdisc.keys():
        ...
        for ii in range(len(segments)):
            tt = find_line_terminals(tmnl, segments)
            tmnl = tt
            if tt:
                for jj in tt:
...
def line_direction_topology(ftnodes):
...
def FnTnodeID(inshp, isterminal=False, updatefield=True):
    """Generate from-node and to-node id for each line segment"""
    segment_counts, line_nodes = comt.reading_polyline(inshp)
    nodeids, terminal = comt.line_fnt_nodes(line_nodes, segment_counts)
    if updatefield == True:
        # update shapefile
        driver = ogr.GetDriverByName('ESRI Shapefile')
        dataSource = driver.Open(inshp, 1)
        layer = dataSource.GetLayer()
        unqfieldname = comt.unique_and_newfield(layer, "FNODE")
        _ = comt.unique_and_newfield(layer, "TNODE")
        # update segments
        if unqfieldname:
            for u in range(layer.GetFeatureCount()):
                feature = layer.GetFeature(u)
...
def CreateGroupID(inlineshp, fieldname="GROUPID"):
    """Generate group id for connected lines"""
    driver = ogr.GetDriverByName('ESRI Shapefile')
    dataSource = driver.Open(inlineshp, 1)
    layer = dataSource.GetLayer()
    # shapefile attribute updating
    segment_counts, _ = comt.reading_polyline(inlineshp)
    ftnodes = FnTnodeID(inlineshp, updatefield=False)
    group_1, group_2 = group_segments(ftnodes, segment_counts)
    unqfieldname = comt.unique_and_newfield(layer, fieldname)
    if unqfieldname:
        for u in range(layer.GetFeatureCount()):
            feat = layer.GetFeature(u)
            comid = feat.GetField(unqfieldname)
            update_field(group_1, group_2, layer, feat, comid, fieldname)
    else:
        for u in range(layer.GetFeatureCount()):
            feat = layer.GetFeature(u)
            comid = u+1
...
def GenerateHydroID(infc, groupid="GROUPID", outfield="HydroID"):
    """Update field"""
    # reading shapefile
    driver = ogr.GetDriverByName("ESRI Shapefile")
    ...
    gdisc = comt.reading_polyline(infc, fieldname=groupid)
    unqfieldname = comt.unique_and_newfield(layer, outfield)
    hydroids = generate_hydroid(gdisc)
    # update segments
    if unqfieldname:
        hydid = 1
        for f in hydroids:
...
def LineDirectionError(infc):
    """Get all line directional error"""
    # streamline nodes accessing
    objects, nodes = comt.reading_polyline(infc)
    tonodes1 = FnTnodeID(infc)
    # flip line
    revered_nodes = []
    rsegment_counts = []
    ...
    tonodes2, _ = comt.line_fnt_nodes(revered_nodes, rsegment_counts)
    # line dirrectional error
    from_direction_topology = line_direction_topology(tonodes2)
    to_direction_topology = line_direction_topology(tonodes1)
